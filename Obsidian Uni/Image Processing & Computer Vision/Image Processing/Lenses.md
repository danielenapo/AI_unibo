#image_processing 
## Depth of Field (DOF)
The pinhole camera model has infinite DOF (everithyn is on focus on everty distance), but it allows little lights in (requires long exposure times, and that leads to motion blur).
